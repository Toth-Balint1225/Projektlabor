...
public class AreaChart extends Pane {
    ...
    public AreaChart(ArrayList<DataSeries> list){
    ...
    public Polygon seriesGen(DataSeries ds){
        ...
        points[points.length-6] = rightPix; points[points.length-5] = botPix; points[points.length-4] = leftPix; 
        points[points.length-3] = botPix; points[points.length-2] = leftPix; 
        points[points.length-1] = botPix-(((data[0]+Math.abs(minValue))/yScaling)*(botPix-topPix));
        
        Polygon poly = new Polygon(points);
        poly.setStroke(Color.BLACK);
        poly.setFill(ds.getColor());
        poly.setOpacity(0.8);
