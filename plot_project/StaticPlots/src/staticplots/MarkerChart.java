 ...
 * @author tobak11
 */
public class MarkerChart extends Pane {
    ...
    //Marker lists
    ArrayList<Rectangle> rectMarkers;
    ArrayList<Circle> circMarkers;
    
    //MarkerCountrer (TEMPORARAY SOLUTION)
    int markerCounter;
    ...
    public MarkerChart(ArrayList<DataSeries> list){
        ...
        rectMarkers = new ArrayList<>();
        circMarkers = new ArrayList<>();
        ...
        for(int i=0;i<dsList.size();i++){
            markerGen(dsList.get(i));
        }
        
        this.getChildren().addAll(rectMarkers); this.getChildren().addAll(circMarkers);
        ...
        setGraphLabels("Time", "Values", "Random data"); this.getChildren().addAll(xAxisLabel, yAxisLabel, graphLabel);
    }
    
    public void markerGen(DataSeries ds){
        ...
        for(int i=0;i<dataLength;i++){
            ...
            if(markerCounter==0){
                Rectangle tempRect = new Rectangle(points[2*i]-4,points[2*i+1]-4,8,8);
                tempRect.setStroke(ds.getColor());
                tempRect.setFill(Color.WHITE);
                
                rectMarkers.add(tempRect);
            }else{
                Circle tempCirc = new Circle(points[2*i],points[2*i+1],4);
                tempCirc.setStroke(ds.getColor());
                tempCirc.setFill(Color.WHITE);
                
                circMarkers.add(tempCirc);
            }
    ...
    public void labelGen(){
        ...
        for(int i=0;i<yTickCounter+1;i++){
            if(botPix-i*yIncrementer>=topPix){
                Text temp = new Text(String.valueOf((int)(Math.floor(minValue)+i*wantedStepY)));
